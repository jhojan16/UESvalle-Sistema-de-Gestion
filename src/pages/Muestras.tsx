    ...
    <Grid size={{ xs: 12, md }}>
        <Box sx={{ mb: 1 }}>
            ...
            <Typography variant="body1" sx={{ minHeight: '1.5em' }}>
                {value ?? <Typography component="span" variant="body2" color="text.secondary">No registrado</Typography>}
            </Typography>
        </Box>
    </Grid>
);
...
export default function VistaAnalisisMuestras() {
    ...
    const columns: GridColDef[] = [
        {
            field: 'muestra_no', headerName: 'Nº Muestra', flex: 1, minWidth: 100, 
            renderCell: (p) => <Chip label={p.value || 'Sin número'} color="primary" size="small" variant="outlined" />,
        },
        {
            field: 'fecha_toma', headerName: 'Fecha Toma', flex: 1, minWidth: 100,  
            renderCell: (p) => p.value ? new Date(p.value).toLocaleDateString('es-CO') : '-',
        },
        { field: 'tipo_muestra', headerName: 'Tipo muestra', flex: 0.5, minWidth: 100,  },
        { field: 'irca', headerName: 'IRCA', flex: 0.5, minWidth: 100, },
        {
            ...
            getActions: (p) => [
                <GridActionsCellItem
                    icon={<Eye size={20} />}
                    label="Ver Detalles"
                    onClick={() => { setSelectedAnalisis(p.row); setDialogOpen(true); }}
                />,
            ],
        },
    ];
    ...
    const closeDialog = () => { setDialogOpen(false); setSelectedAnalisis(null); };

    return (
        <Box>
            <Box sx={{ mb: 4 }}>
                <Typography variant="h4" fontWeight="bold" gutterBottom>Análisis de Muestras</Typography>
                <Typography variant="body1" color="text.secondary">Gestión y consulta de calidad del agua</Typography>
            </Box>
            ...
            <Dialog open={dialogOpen} onClose={closeDialog} maxWidth="xl" fullWidth>
                <DialogTitle>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Box>
                            <Typography variant="h5" fontWeight="bold">Detalles del Análisis</Typography>
                            <Typography variant="body2" color="text.secondary">Muestra: {selectedAnalisis?.muestra_no || 'Sin número'}</Typography>
                        </Box>
                        ...
                        <Box sx={{ py: 1 }}>
                            <InfoSection title="Información General">
                                {CONFIG_CAMPOS.general.map(f => (
                                    <InfoField key={f.label} label={f.label} value={selectedAnalisis[f.key as keyof MuestraCompleta]} md={f.md} />
                                ))}
                            </InfoSection>
                            ...
                            <InfoSection title="Punto de Muestreo">
                                {CONFIG_CAMPOS.punto.map(f => (
                                    <InfoField key={f.label} label={f.label} value={selectedAnalisis.punto_muestreo?.[f.key as keyof typeof selectedAnalisis.punto_muestreo]} md={f.md} />
                                ))}
                            </InfoSection>

                            <InfoSection title="Prestador">
                                {CONFIG_CAMPOS.prestador.map(f => (
                                    <InfoField key={f.label} label={f.label} value={selectedAnalisis.prestador?.[f.key as keyof typeof selectedAnalisis.prestador]} md={f.md} />
                                ))}
                            </InfoSection>

                            <InfoSection title="Laboratorio">
                                {CONFIG_CAMPOS.laboratorio.map(f => (
                                    <InfoField key={f.label} label={f.label} value={selectedAnalisis.laboratorio?.[f.key as keyof typeof selectedAnalisis.laboratorio]} md={f.md} />
                                ))}
                            </InfoSection>
                        </Box>
                    ) : (
